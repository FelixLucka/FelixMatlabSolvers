function [v_best, z, w, info] = TV_FlowEstimation_ADMM(a, f, v, z, w, lambda, para)
...
%   min_v 1/p \| a .* v - f \|_p^p + lambda * TV(v) 
%   using ADMM
%
% USAGE:
...
%
% INPUTS:
% 	a1 - 2D/3D vector field (size [n_x, n_y, 2] or [n_x, n_y, n_z, 3])
%   f  - 2D/3D vector field (size a1, v)
...
%
% OUTPUTS:
%       v_best - motion field with lowest energy found
...
over_relax_para  = checkSetInput(para, 'overRelaxPara', '>0', 1.8);
accelerate       = checkSetInput(para, 'accelerate','logical', false);
compute_energy     = checkSetInput(para, 'computeEnergy', 'logical', true);
compute_ini_energy = checkSetInput(para, 'computeIniEnergy', 'logical', false);

info = [];

if(~any(f(:)) && ~compute_ini_energy)
...
cast_variables = {'a','f','v','z','w','rho','over_relax_para','lambda'};
myDisp([output_pre_str 'casting variables to type: ' data_cast], output)
for cast_index = 1:length(cast_variables)
    if(eval(['iscell(' cast_variables{cast_index} ')']))
        for i_dim=1:length(cast_variables{cast_index})
            eval([cast_variables{cast_index} '{i_dim} = castFun(' cast_variables{cast_index} '{i_dim});']);
        end
    else
...
if(isempty(z))
    ...
    if(split_OF)
       z_OF     = castZeros(dim_f);
       z_OF_hat = z_OF; % accelerated variable
    end
else
    if(split_OF)
       z_jac = z{1};
...
if(isempty(w))
    ...
    if(split_OF)
        w_OF = castZeros(dim_f);
        w_OF_hat = w_OF; % accelerated variable
    end
else
    if(split_OF)
       w_jac = w{1};
...
if(damping_para > 0)
    ...
    damping_value = max(abs(a(:))) * damping_para;
else
    damping_value = 0;
end
...
switch ls_solver
    ...
    case 'minresMat'
        ...
        pre_con_fLag  = checkSetInput(ls_solver_para, 'preConFL', 'logical', true);
        pre_con_options = checkSetInput(ls_solver_para, 'preConOptions', 'struct', emptyStruct);
        if(pre_con_fLag)
            pre_con_options.type     = checkSetInput(pre_con_options, 'type',...
                {'nofill','ict'}, 'nofill');
            pre_con_options.droptol  = checkSetInput(pre_con_options, 'droptol', '>0', 10^-2);
            pre_con_options.michol   = checkSetInput(pre_con_options, 'michol', ...
                {'on','off'}, 'off');
            pre_con_options.diagcomp = checkSetInput(pre_con_options, 'diagcomp',...
                '>=0', 0);
        end
        ...
        if(cond_est_adaptation)
            ...
            while(cond_est_GG_mat > cond_est_max && rho_adaptation)
                rho_old              = rho;
                % try larger rho
                rho                 = tau * rho_old;
                [GG_mat, i_chol_GG_mat] = assembleGGMat();
...
if(compute_energy)
    ...
    if(any(v(:)))
        OF_discrepancy = 1/p * sumAll(abs(sum(a .* v, dim_space+1) - f).^p);
        TVv            = TVofVelocityField(v, TV_type, true);
    else
        OF_discrepancy = 1/p * sumAll(abs(f).^p);
        TVv            = castZeros(1);
    end
...
while((n_eval < min_eval) || ((n_eval < max_eval) && ~stop))
    ...
    switch ls_solver
        ...
        case 'minres'
            ...
            [v, ~, rel_res_ls, ls_iter, resvec_minres, res_vec_cg_minres] = ...
        ...
        case 'minresMat'
            ...
            [v, flag_minres, rel_res_ls, ls_iter, resvec_minres, res_vec_cg_minres] = ...
        ...
        case 'AMG-CG'
            ...
            [v, amgInfo] = amg(GG_mat,b_minres(:), amg_opts, AMGvar{1}, ...
                AMGvar{2}, AMGvar{3}, AMGvar{4}, AMGvar{5}, AMGvar{6});
            ls_iter       = amgInfo.itStep;
            rel_res_ls     = amgInfo.stopErr;
            n_eval        = n_eval + 2* ls_iter;
            v            = reshape(real(v),dim_v);
            clear *_minres
    end
    ...
    switch ls_stop_criterion
        case 'progRelRes'
            ...
            output_ls = ['itLS: ' int2str(ls_iter) ', rrLS: '  num2str(rel_res_ls,'%.2e')];
        case 'maxIter'
    ...
    %%% over-relaxation
     % F(z) = - z
    aux_var_jac = over_relax_para * Ev_jac + (1-over_relax_para) * z_jac_hat;
    if(split_OF)
        % E(u) + F(z) = b, F(z) = -z, b = -ut
        aux_var_OF = over_relax_para * Ev_OF + (1-over_relax_para) * (z_OF_hat + f); 
    end
    ...
    if(split_OF)
        z_OF = getfield(proxLp(aux_var_OF - f + w_OF_hat, 1/rho, p), 'x');
    end
    ...
    if(split_OF)
        w_OF = w_OF_hat + (aux_var_OF - z_OF - f); % 
    end
    
    if(cmp_primal_dual_res)
        % compute primal and dual residuum
        if(split_OF)
            primal_res_norm = sqrt(sumAll((Ev_jac - z_jac).^2) + sumAll((Ev_OF - z_OF - f).^2));
            dual_res_norm   = rho * sqrt(sumAll((spatialFwdJacobian(z_jac-z_jac_old, true, true) ...
                                            + bsxfun(@times, a, z_OF-z_OF_old)).^2));
        else
    ...
    if(accelerate)
        ...
        if(split_OF)
            w_OF_hat  = w_OF + gamma * (w_OF - w_OF_old);  
            z_OF_hat  = z_OF + gamma * (z_OF - z_OF_old);  
        end
    else
        z_jac_hat  = z_jac;
        w_jac_hat  = w_jac;
    ...
    clear z_jac_old w_jac_old z_OF_old w_OF_old
    ...
    switch stop_criterion
        case 'energyDec'
            if(~stop && min_energy < energy(1))
                firstEnergyDec = find(energy(2:end) <= energy(1),1,'first') + 1;
                if(~isempty(firstEnergyDec)  && firstEnergyDec <= ceil(length(energy)/energy_overshot))
                    stop = true;
                end
            end
        case 'maxEval'
...
if(return_algo_var)
    if(split_OF)
        ...
        z{2} = gather(z_OF); clear z_OF
        w{1} = gather(w_jac); clear w_jac
        w{2} = gather(w_OF); clear w_OF
    else
...
info.iter          = iter;
info.rho           = gather(rho);
info.rhoAdaptation = rho_adaptation;
...
% =========================================================================
    ...
    function res = GGfun(vIn)
        ...
        if(split_OF)
            res = res + bsxfun(@times, a, sum(a .* vIn, dim_space+1));
            if(damping_para > 0)
                res = res + damping_value^2 * vIn;
            end
        else
            res = res + bsxfun(@times, a, sum(a .* vIn, dim_space+1)) / rho;
            if(damping_para > 0)
            ...
            case 'minresMat'
                if(pre_con_fLag)
                    while(1)
                        ...
                        try
                            dec_GG_mat = ichol(GG_mat, pre_con_options);
                            break
                        catch ME
                            switch ME.message
                                case 'Encountered nonpositive pivot.'
                                    pre_con_options.diagcomp = max(10^-10, 13*pre_con_options.diagcomp);
                                otherwise
                                    rethrow(ME)
                            end
                        end
                    end
                    pre_con_options.diagcomp = pre_con_options.diagcomp/11;
                end
            case 'AMG-CG'
    ...
    function CGsolve()
        ...
        if(split_OF)
            r_cg     = r_cg + bsxfun(@times, a, z_OF_hat + f - w_OF_hat);
            if(damping_para > 0)
                r_cg = r_cg + damping_value^2 * v_start;
            end
        else
            r_cg     = r_cg + bsxfun(@times, a, + f/rho);
            if(damping_para > 0)
    ...
    function b_minres = setupRHS()
        ...
        if(split_OF)
            b_minres = b_minres + bsxfun(@times, a, z_OF_hat + f - w_OF_hat);
            if(damping_para > 0)
                b_minres = b_minres + damping_value^2 * v_start;
            end
        else
            b_minres = b_minres + bsxfun(@times, a, + f/rho);
            if(damping_para > 0)
